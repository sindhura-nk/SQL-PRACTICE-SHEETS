...
*/
CREATE CLUSTERED INDEX IND_EMPLOYEE_ID ON EMPLOYEES(EMPLOYEEID);
...
-- TO VIEW THE ESTIMATED COST, CTRL+ L
...
CREATE NONCLUSTERED INDEX IND_DEPT ON EMPLOYEES(DEPARTMENT);
...
*/
CREATE NONCLUSTERED INDEX IND_DEPT_MKT ON EMPLOYEES(DEPARTMENT)
WHERE DEPARTMENT='MARKETING';

-- Find out Marketing department wise count of employees
SELECT COUNT(*) AS TOTAL_COUNT FROM EMPLOYEES WHERE Department='MARKETING';
...
Department + Salary
*/
CREATE NONCLUSTERED INDEX IND_DEPT_SALARY ON EMPLOYEES(DEPARTMENT,SALARY);

EXEC SP_HELPINDEX 'EMPLOYEES';
-- DISPLAY RECORD OF EMPLOYEE FROM FINANCE SALARY >70000;
SELECT * FROM Employees WHERE Department='FINANCE' AND SALARY>70000;
